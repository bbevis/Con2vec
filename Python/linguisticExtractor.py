...
async def transcribe_audio_with_deepgram(audio_path):
...
def extract_transcription_data(transcription_result):
...
def save_transcriptions_to_csv(transcriptions, output_csv, filename):
...
async def transcribe_single_file(input_audio_file, output_csv_file):
    # Perform transcription using Deepgram
    print(f"Processing {input_audio_file}...")
    print('')
    transcription_result = await transcribe_audio_with_deepgram(input_audio_file)
    transcription_data = extract_transcription_data(transcription_result)
    
    # Save the results to CSV
    save_transcriptions_to_csv(transcription_data, output_csv_file, os.path.basename(input_audio_file))
